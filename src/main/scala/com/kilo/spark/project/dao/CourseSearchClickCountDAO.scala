...
import com.kilo.spark.project.domain.CourseSearchClickCount
...
object CourseSearchClickCountDAO {
  ...
  def save(list: ListBuffer[CourseSearchClickCount]): Unit = {
    ...
    for (ele <- list) {
      table.incrementColumnValue(ele.day_search_course.getBytes(), cf.getBytes(), qualifier.getBytes(), ele.click_count)
    }
  }
